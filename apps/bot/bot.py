...
class ProfitHubBot:
    ...
    async def simple_color_analysis_with_vision(self, image_data):
        """Упрощенный анализ цветов с использованием VisionAnalyzer - ИСПРАВЛЕННЫЙ"""
        try:
            from PIL import Image
            import io
            import numpy as np

            # Сбрасываем позицию и создаем копию
            ...
            if hasattr(image_data, 'read'):
                ...
                image_data_copy = io.BytesIO(image_data)
            ...
            # Используем исправленный универсальный анализ
            try:
                from parser.utils.vision_analyzer import vision_analyzer

                # Получаем анализ цветов
                color_analysis = vision_analyzer.analyze_colors_universal(image_cv)

                if color_analysis and 'colors' in color_analysis:
                    # Форматируем результат для совместимости
                    dominant_colors = []
                    for color_info in color_analysis.get('palette', []):
                        dominant_colors.append({
                            'rgb': color_info.get('rgb', [128, 128, 128]),
                            'name': color_info.get('name', 'основной'),
                            'percentage': color_info.get('percentage', 20.0),
                            'hex': color_info.get('hex', '#808080')
                        })

                    # Простое разделение фона и объекта
                    background_color = dominant_colors[0] if dominant_colors else None
                    object_colors = dominant_colors[1:3] if len(dominant_colors) > 1 else []
                    ...
                    return {
                        ...
                        'color_palette': {
                            'type': 'универсальная',
                            'tone': 'средняя',
                            'saturation': color_analysis.get('saturation', 128),
                            'brightness': color_analysis.get('brightness', 128),
                            'saturation_level': color_analysis.get('saturation_level', 'средняя'),
                            'brightness_level': color_analysis.get('brightness_level', 'нормальная')
                        },
                        'image_size': image_size,
                        'colors_count': color_analysis.get('colors_count', 1)
                    }
                else:
                    # Fallback если анализ не удался
                    return await self.basic_color_analysis(image, image_size)

            except Exception as vision_error:
                logger.warning(f"⚠️ Продвинутый анализ недоступен: {vision_error}")
                # Используем базовый анализ как запасной вариант
                return await self.basic_color_analysis(image, image_size)
    ...
    async def _analyze_colors_advanced(self, image_cv, k=6):
        """Продвинутый анализ цветов с выделением основных - использует универсальный метод"""
        try:
            from parser.utils.vision_analyzer import vision_analyzer
            if hasattr(vision_analyzer, 'analyze_colors_universal'):
                return vision_analyzer.analyze_colors_universal(image_cv)
            else:
                logger.warning("⚠️ Метод analyze_colors_universal недоступен")
                return await self._fallback_color_analysis(image_cv)
        except Exception as e:
            logger.error(f"❌ Ошибка доступа к vision_analyzer: {e}")
            return await self._fallback_color_analysis(image_cv)

        # В методе _analyze_color_palette замените:
        return vision_analyzer.analyze_color_palette(image_cv)

        # На:
        try:
            from parser.utils.vision_analyzer import vision_analyzer
            if hasattr(vision_analyzer, 'analyze_color_palette'):
                return vision_analyzer.analyze_color_palette(image_cv)
            else:
                logger.warning("⚠️ Метод analyze_color_palette недоступен")
                return {
                    'type': 'упрощенная',
                    'tone': 'средняя',
                    'saturation': 128,
    ...
    async def _analyze_color_palette(self, image_cv):
        """Анализирует цветовую палитру изображения"""
        try:
            from parser.utils.vision_analyzer import vision_analyzer
            return vision_analyzer.analyze_color_palette(image_cv)
        except Exception as e:
            logger.error(f"❌ Ошибка анализа палитры: {e}")
            return {
                'type': 'упрощенная',
                'tone': 'средняя',
                'saturation': 128,
    ...
    def _get_detailed_color_name(self, rgb_color):
        """Детальное определение названия цвета"""
        try:
            from bot.services.color_analyzer import color_analyzer
            return color_analyzer._get_detailed_color_name(rgb_color)
        except:
            return "основной"

    async def _separate_background_and_object(self, image_cv, dominant_colors):
        """Разделяет фон и объект на изображении - ИСПРАВЛЕННАЯ ВЕРСИЯ"""
        try:
            import cv2
            import numpy as np
            ...
            if image_cv is None or image_cv.size == 0:
                logger.warning("⚠️ Пустое изображение для разделения фона")
                return None, []

            # Простая логика: самый распространенный цвет - фон
            if dominant_colors:
                background_color = dominant_colors[0]  # Самый доминирующий цвет

                # Объектные цвета - все остальные значимые цвета
                object_colors = dominant_colors[1:3] if len(dominant_colors) > 1 else []

                # Убедимся что цвета имеют правильную структуру
                ...
                return bg_color, obj_colors
            else:
                return None, []

        except Exception as e:
            logger.error(f"❌ Ошибка разделения фона и объекта: {e}")
            return None, []

    async def basic_color_analysis(self, image, image_size):
        """Базовый анализ цветов как запасной вариант"""
        try:
            import numpy as np
            ...
            # Берем средний цвет изображения
            avg_color = np.mean(image_np.reshape(-1, 3), axis=0).astype(int)

            # Создаем упрощенный результат с одним доминирующим цветом
            dominant_colors = [{
                'rgb': avg_color.tolist(),
                'name': self._get_basic_color_name(avg_color),
                'percentage': 100.0,
    ...
    def _get_basic_color_name(self, rgb_color):
        """Базовое определение названия цвета"""
        try:
            r, g, b = rgb_color
